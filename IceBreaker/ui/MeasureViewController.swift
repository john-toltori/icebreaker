...
class MeasureViewController: UIViewController, ProtocolDelegate, BLEDataProcessDelegate {
    ...
    @IBOutlet weak var stPinNumber: UIStepper!
    ...
    let SENSOR_PIN_FROM: UInt8 = 11
    let SENSOR_PIN_TO: UInt8 = 19
    ...
    var pinAnalog: [UInt16] = [UInt16](count: 128, repeatedValue: 0)
    ...
    */
    ...
    @IBAction func onPinNumber_ValueChanged(sender: AnyObject) {
        SENSOR_PIN = UInt8(stPinNumber.value)
        txtPinNumber.text = "\(SENSOR_PIN)"
    }
    ...
    func protocolDidReceiveTotalPinCount(count: UInt8) {
        totalPinCount = count
        for var i: UInt8 = SENSOR_PIN_FROM; i < SENSOR_PIN_TO; i++ {
            rblProtocol.setPinMode(i, mode: UInt8(ANALOG))
        }
